...
- The pointer returned by `mmap` is valid for the lifetime of `ShmRegion`
- No other process will modify the layout unexpectedly (enforced by mutex)
- The region must be at least `size_of::<ShmStateLayout>()` bytes
